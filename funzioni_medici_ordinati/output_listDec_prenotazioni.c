                                  ...
                                  int lista_prenotazioni_SenzaDuplicati[], int counter_somma_prenotazioni, int new_lista_prenotazioni[] ) 
{
  ...
  puts( " " );
  puts( "LISTA MEDICI IN ORDINE DECRESCENTE PER NUMERO DI PRENOTAZIONI" );
  
  for( int i = counter_somma_prenotazioni - 1; i >= 0; i-- ) { // finchè il valore di i è maggiore o uguale a zero 
    for( int j = 0; j < n_medici; j++  ) { // finchè il valore di j è minore del numero di medici memorizzati nel file binario "VISITE"
